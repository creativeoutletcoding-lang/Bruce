...
import type { NormalizedMessage, WorkingLogEntry } from "@/lib/chat/types";

export function normalizeMessage(raw: Record<string, unknown>): NormalizedMessage {
  const metadata = (raw.metadata as Record<string, unknown> | null | undefined) ?? null;
  const workingLog = metadata?.working_log;
  return {
    id: raw.id as string,
    ...
    metadata,
    working_log: Array.isArray(workingLog) ? (workingLog as WorkingLogEntry[]) : null,
  };
}
