...
### Structural working-log routing, smooth streaming, Drive hygiene, instruction guard — 2026-06-10

**Working-log routing is architectural, not prompt-based.** Production showed `TASK_PROGRESS_SYSTEM_BLOCK`'s "zero text during tool calls" rule failing — inter-tool narration ("Let me read the sheet…", "Got it — rows 43 and 44…") streamed into the visible reply and was persisted into `messages.content`. The fix routes assistant content by block position inside `runChatStream`: any text block followed by a `tool_use` (or `server_tool_use`) in the same turn is working narration; the final text block is the reply. Routing happens on SDK `content_block_start` boundaries — a model compliance failure can now only narrate into the working log, never the reply bubble. The prompt block is retained purely as a volume reducer.

**Wire protocol.** Narration streams on the default text channel; when a tool block starts, the server emits `\x1eNARRATION_BREAK\x1e` and the client (single-pass tokenizer in `parseStreamFrame`) moves everything since the previous break into the working log. With the smooth-reveal renderer the pre-tool narration usually never visibly renders in the bubble before being rerouted. If the final turn produced no text, `\x1ePROMOTE_LAST\x1e` promotes the last narration segment back to the reply so Bruce never says nothing.

**Persistence.** `messages.content` = final reply only. Narration + tool calls live in ordered `metadata.working_log` (`{narration|tool_use|tool_result}`, chronological, tool_trace-style caps: 600 chars/entry, 24 entries) — this **replaces** `tool_trace` for new rows; `formatAssistantReplay` reads both, preferring `working_log` (narration included for grounding). No migration — metadata is jsonb; old rows with narration baked into content render as-is.

**UI.** One collapsed expandable `WorkingLog` container — under the task card in `StreamingStatusBar` while streaming (mirroring where the live task card already lives), and in the message flow (under the task card, or standalone above the reply in `MessageBubble`) after completion. `normalizeMessage` → `workingLogToDisplay` reconstructs it from metadata so history reload renders identically to live in all chat contexts.

**Smooth streaming renderer.** `consumeStream` (the single shared consumer) now buffers network chunks and reveals them via a rAF loop at an adaptive character cadence: reveal rate scales with backlog so rendering never lags the network by more than ~1.5s and never crawls on long replies; stream end and Stop flush instantly. Sentinels traverse the reveal cursor atomically at zero cost (`advanceReveal`, pure + unit-tested) so task ticks/statuses stay live and a sentinel split across chunks can never leak as text. Narration sweeps at a 4× lighter cadence. Implemented inside `consumeStream` rather than a per-context React hook deliberately — all four contexts get it with no wiring, per the CHAT LOGIC rule.

**Literal-markdown bug (root cause).** Assistant messages under a completed task card rendered raw `**asterisks**` on history reload: `MessageList`'s task branch rendered plain `pre-wrap` text while live streaming rendered through `marked()` in `MessageBubble`. Both paths now use the shared `AssistantMarkdown` component (`useMemo`-throttled parse). `linkifyText` was investigated and ruled out — it only touches human messages.

**Drive hygiene.** New `trash_drive_file` tool (Drive `files.update {trashed: true}`, 30-day recoverable), with ID-type guardrails in both directions: its description pins it to Drive file IDs only, and `delete_email`'s description now says Gmail message IDs only — production had `delete_email` misused on Drive file IDs (Gmail 400). `generate_csv` is now overwrite-in-place — an explicit `file_id` or a same-named non-trashed file in the target folder is updated via media upload (same fileId) instead of duplicated (production had four `payroll_06152026.csv` copies). Result reports `action: created|updated`, which makes the CPS "overwrite it silently" instruction structurally true.

**Instruction guard (CPS truncation incident).** The unmount-fired auto-updater (`/api/projects/[id]/instructions/update`) had asked Sonnet for the *full rewritten instructions* at `max_tokens: 1024` and silently overwrote the row — the CPS instructions were truncated to 3,474 chars mid-list, losing output specs and the verification rule. The route is now **append-only**: the model returns a short addendum or `NO_CHANGE`; the write is always `current + "\n\n" + addendum`; a tripwire blocks any shrinking write. Every instructions write — auto and manual panel edits (which may legitimately shorten and are logged, not blocked) — is recorded to `system_config` key `instructions_audit_log` (timestamp, project, chat, source, old/new lengths, capped at 100) plus Vercel logs, so any future truncation is traceable. `system_config` was chosen over a new audit table to avoid a manual migration.
