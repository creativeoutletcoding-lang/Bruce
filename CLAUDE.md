...
Migrations 001–035 all applied (verified against prod 2026-06-10; see `docs/migration-log.md`). `schema.sql` is the source of truth — always update it when altering structure. Current table reference incl. `messages.metadata.working_log` shape: `docs/schema-summary.md`.
...
**Document tools** — `lib/documents/documentTools.ts`. Read spreadsheets, CSVs, and Drive files; resolve paths; list directory contents; trash Drive files. `generate_csv` **overwrites in place** — an explicit `file_id` or a same-named non-trashed file in the target folder is updated (same fileId) instead of duplicated; result reports `action: created|updated`. `trash_drive_file` moves a file to Drive trash (30-day recoverable) — Drive file IDs only, never Gmail message IDs (and `delete_email` is Gmail message IDs only, never Drive file IDs); confirm before trashing user-created files. System block: `DOCUMENT_SYSTEM_BLOCK`. All contexts.
...
**Multi-step task progress** — For tasks with 3+ sequential tool calls, Bruce emits `<task_progress>` XML blocks that render as live progress cards on the client. No intermediate text or calculations during tool execution — only the task card and a brief final summary. Defined via `TASK_PROGRESS_SYSTEM_BLOCK` in `lib/anthropic/index.ts`. All contexts. **Note:** narration containment is no longer prompt-dependent — structural working-log routing (below) is the guarantee; this block only reduces narration volume.

**Working-log routing (structural)** — `runChatStream` routes assistant content by block position: any text block followed by a `tool_use`/`server_tool_use` in the same turn is working narration; the final text block is the reply. Implemented on SDK `content_block_start` boundaries via `\x1eNARRATION_BREAK\x1e` sentinels (`\x1ePROMOTE_LAST\x1e` promotes the last narration segment if the final turn has no text — Bruce never says nothing). `messages.content` persists the reply ONLY; narration + tool steps live in ordered `metadata.working_log`. Client: `parseStreamFrame` single-pass tokenizer → `StreamTick.workingLog`; collapsed expandable `WorkingLog` container renders under the task card (live in `StreamingStatusBar`, persisted in `MessageList`/`MessageBubble`); history reload reconstructs identically via `normalizeMessage` → `workingLogToDisplay`. Core module: `lib/chat/workingLog.ts`. No-tool turns are untouched (single text block = reply, no container).
...
| Client stream consumer + finalizer (smooth reveal, abort, image-req, final text/task/working-log) | `lib/chat/clientStream.ts` |
| Working-log types, recorder, display mapping | `lib/chat/workingLog.ts` |
| Working-log container UI (collapsed expandable) | `components/chat/WorkingLog.tsx` |
| Assistant markdown renderer (single path, live = history) | `components/chat/AssistantMarkdown.tsx` |
...
**Streaming model:** the server emits one consolidated assistant message per turn even when tools interleave (no per-turn DB rows); `messages.content` is the final reply only, narration goes to `metadata.working_log` (structural routing — see Active Tools). The client consumes the stream through `consumeStream()`, which buffers network chunks and reveals them via a rAF loop at an adaptive character cadence (rate scales with backlog; never lags the network by more than ~1.5s, flushes instantly at stream end; sentinels traverse atomically via `advanceReveal`). Markdown parses from the revealed buffer at most once per frame through the shared `AssistantMarkdown` component — never per network chunk, and identically for live and history renders. An `AbortController` from `ChatWindow`/`ProjectChatView`/`FamilyChatWindow` is passed both to `fetch` and `consumeStream` — pressing Stop in `MessageInput` cancels both, preserves whatever text has been emitted, flushes in-flight narration to the working-log container, and marks any in-flight task steps as cancelled.
...
- **Working log:** `runChatStream` persists narration + tool calls in ordered `messages.metadata.working_log` (`{narration|tool_use|tool_result}`, capped via `lib/chat/toolTrace.ts` / built by `lib/chat/workingLog.ts`); the three chat routes replay it through `formatAssistantReplay()` so follow-up turns keep tool grounding (legacy `metadata.tool_trace` rows still replay). New chat routes must do the same.
...
**Working-log routing + instruction guard (2026-06-10):** Narration containment is structural — `runChatStream` routes text by block position (pre-tool text → `metadata.working_log`, final text block → `content`), never by prompt compliance. The project-instructions auto-updater is append-only with a shrink tripwire and full audit logging (`system_config` key `instructions_audit_log`) after the CPS truncation incident. `generate_csv` overwrites in place; `trash_drive_file` added with Drive/Gmail ID-type guardrails.
