...
**Engagement decision (shared, speaker-aware):** `lib/chat/engagement.ts` (`decideEngagement`) is the **canonical multi-member awareness mechanism** — the speaker-aware, context-aware judgment of whether a group message is addressed to Bruce. **Both `/api/family/chat` AND group (multi-member) project chats `/api/projects/[id]/chat` call it** (convergence-spec fork #2 resolved). Group projects are **no longer always-respond** — Bruce stays silent during member-to-member chatter and only replies when addressed. **Single-member projects keep the always-respond contract (no gate);** the discriminator is `shouldEngagementGate(memberCount)` (`> 1`). Standalone/NewChat untouched. The route-side plumbing both routes share — `buildEngagementHistory` (speaker-labeled history, assistant tool-trace replayed so pending questions survive), `buildNameForSender` (service-role name resolution, null→`"Bruce"`, unknown→`"Member"`), and `shouldEngagementGate` — lives in `lib/chat/engagementContext.ts` so there is exactly **one** history-labeler, not a copy per route. It returns the unchanged outcome set RESPOND / REACT_THUMBS / REACT_HEART / SILENT; react decisions insert a Bruce reaction via `executeReactionTool` and return `X-Bruce-Responded: false` (no model turn); silent returns `X-Bruce-Responded: false` with no stream body and no persisted assistant row; classifier failure falls back to SILENT. In the project route the decision is kicked off after history is built and **awaited at a gate placed before the expensive respond-path work** (Drive file-content load, memory, system prompt), so a silent group message persists the member's message + fires notifications but triggers no Drive I/O. Clients (`FamilyChatWindow`, `ProjectChatView`) read `X-Bruce-Responded`: on `!== "true"` they drop the optimistic `tmp-stream-` placeholder and bail (the post-send reload is authoritative). Three properties beyond a bare name-match gate: (1) **speaker-aware history** — the `HAIKU_MODEL` classifier sees real per-message sender names (`"Laurianne:"` / `"Bruce:"`), never a flattened `"Member"`. (2) **Open-question window** — `OPEN_QUESTION_WINDOW = 3` (member messages): within that many member messages of one of Bruce's questions/proposals, a nameless reply ("yeah", "sure", "go ahead") is given to the classifier *with Bruce's pending question* so answers-to-his-own-questions trigger; outside the window the path is off. Computed **ephemerally from history** (`findPendingOpenQuestion`) — no stored conversational state, no DB column. (3) **Address vs mention** — a bare `/\bbruce\b/` no longer auto-responds; only a clear vocative/@mention (`isStronglyAddressed`) short-circuits, so a third-person mention between members ("did Bruce add it?") routes to the classifier with speaker context and stays silent. Pure helpers are unit-tested in `lib/chat/__tests__/engagement.test.ts`; the shared plumbing + decision wiring in `engagementContext.test.ts` and `engagementDecision.test.ts`.
